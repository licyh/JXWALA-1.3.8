...
public class SWTCallGraph {
  ...
  public static void main(String[] args) throws WalaException {
    ...
    //System.out.println(p.toString());  //by JX   Result:{appJar=Z:\WALA\com.ibm.wala.core.testdata\JLex.jar}
    run(p);
  }
  ...
  public static ApplicationWindow run(Properties p) throws WalaException {
    ...
    try {
      ...
      if (PDFCallGraph.isDirectory(appJar)) {
        ...
        //System.out.println("IN - " + appJar);  //by JX   it didn't get in
      }

      String exclusionFile = p.getProperty("exclusions");
      //System.out.println(exclusionFile);  //by JX    exclusionFile is null
      ...
      //System.out.println("After SCOPE - ");  //by JX
      ClassHierarchy cha = ClassHierarchy.make(scope);
      //System.out.println( cha.getNumberOfClasses() );           //by JX
      
      Iterable<Entrypoint> entrypoints = null;
      JarFile jar = new JarFile(appJar); 
      if (jar.getManifest() != null) {
        String mainClass = jar.getManifest().getMainAttributes().getValue("Main-Class");
        //System.out.println(mainClass);  //by JX    Result: null
        if (mainClass != null) {
      ...
      if (entrypoints == null) {
        ...
        //System.out.println(entrypoints.toString());  //by JX; can't get them though this, 
        //JX: actually two entrypoints: < Application, LJLex/SparseBitSet, main([Ljava/lang/String;)V >   < Application, LJLex/Main, main([Ljava/lang/String;)V >
      }
      
      //JX: the step is important
      AnalysisOptions options = new AnalysisOptions(scope, entrypoints);   //JX: it must needs some entrypoints to begin the analysis. 
      ...
      v.run();
      return v.getApplicationWindow();   //JX: we can remove this.
